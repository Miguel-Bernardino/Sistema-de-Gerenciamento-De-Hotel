...
export const SidebarDesktop: React.FC<any> = ( { focusPage, setFocusPage, ...props }) => {
    ...
    return(
        <div 
            className={`bg-(--sidebar-bg) rounded-r-2xl h-screen flex flex-col items-start py-4 ${isHovered ? style.sidebarRootExpanded : style.sidebarRootCollapsed}`}
            style={{ width: isHovered ? 220 : 50, transition: 'width 220ms cubic-bezier(.2,.9,.2,1)'}}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
            
            <header className="w-full flex items-center justify-center h-20 mb-[clamp(1vh,10px,10vh)]">
                <Chevrons isHovered={isHovered} />
            </header>
            ...
            <main className="w-full">
                <ol className={style.list}>
                    <main className="">
                        {/*------------------HOME PAGE------------------------*/}
                        <li className={`${style["sidebar-li"]} ${isPathActive('/home') ? style["sidebar-li-active"] : ''}`}>    
                            {isPathActive('/home') && <HighlightIndicator inline={isHovered} />}
                            <NavLink to="/home" className="flex items-center h-[50px] w-[50px]" onClick={(e) => { setFocusPage?.('/home') }}>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={style.sidebarItem}>
                                    <path d="M11.47 3.841a.75.75 0 0 1 1.06 0l8.69 8.69a.75.75 0 1 0 1.06-1.061l-8.689-8.69a2.25 2.25 0 0 0-3.182 0l-8.69 8.69a.75.75 0 1 0 1.061 1.06l8.69-8.689Z" />
                                    <path d="m12 5.432 8.159 8.159c.03.03.06.058.091.086v6.198c0 1.035-.84 1.875-1.875 1.875H15a.75.75 0 0 1-.75-.75v-4.5a.75.75 0 0 0-.75-.75h-3a.75.75 0 0 0-.75.75V21a.75.75 0 0 1-.75.75H5.625a1.875 1.875 0 0 1-1.875-1.875v-6.198a2.29 2.29 0 0 0 .091-.086L12 5.432Z" />
                                </svg>
                                <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Início</span>
                            </NavLink>
                        </li>

                        {/*------------------STATISTIC PAGE------------------------*/}
                        <li className={`${style["sidebar-li"]} ${isPathActive('/statistics') ? style["sidebar-li-active"] : ''}`}>
                            {isPathActive('/statistics') && <HighlightIndicator inline={isHovered} />}
                            <NavLink to="/statistics" className="flex items-center" onClick={(e) => { setFocusPage?.('/statistics') }}>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 105.98" fill="currentColor" className={`${style.sidebarItem}`}>
                                ...
                                <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Estatísticas</span>
                            </NavLink>
                        </li>

                        {/*------------------PRODUCTS PAGE------------------------*/}
                        <li className={`${style["sidebar-li"]} ${isPathActive('/products') ? style["sidebar-li-active-product"] : ''}`}>
                            <NavLink to="/products" className="flex items-center" onClick={(e) => { setFocusPage?.('/products') }}>
                                {isPathActive('/products') && <HighlightIndicator inline={isHovered} />}
                                <svg className={style.sidebarItem} viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Produtos</span>
                            </NavLink>
                        </li>

                        {/*---------------PRODUCTS PRICES PAGE--------------------*/}
                        <li className={`${style["sidebar-li"]} ${isPathActive('/finance') ? style["sidebar-li-active-finance"] : ''}`}>
                            {isPathActive('/finance') && <HighlightIndicator inline={isHovered} />}
                            <NavLink to="/finance" className="flex items-center" onClick={(e) => { setFocusPage?.('/finance') }}>
                                <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="-40 -38 192.88 170.81" className={style.sidebarItemFinanceIcon}>
                                ...
                                <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Preços</span>
                            </NavLink>
                        </li>

                        {/*------------------SERVICES PAGE (IN FUTURE)------------------------*/}
            ...
            <footer className="w-full mt-auto">
                <ol className={style.list}>
                    {/*--------------------------CONFIG PAGE------------------------*/}
                    <li className={`${style["sidebar-li"]} ${isPathActive('/config') ? style["sidebar-li-active"] : ''}`}>                        
                        {isPathActive('/config') && <HighlightIndicator inline={isHovered} />}
                        <NavLink to="/config" className="flex items-center" onClick={(e) => { setFocusPage?.('/config') }}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={style.sidebarItem}>
                            ...
                            <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Configurações</span>
                        </NavLink>
                    </li>

                    {/*--------------------------LOGOUT PAGE------------------------*/}
                    <li className={`${style["sidebar-li"]} ${isPathActive('/logout') ? style["sidebar-li-active"] : ''}`}>
                        {isPathActive('/logout') && <HighlightIndicator inline={isHovered} />}
                        <NavLink to="/logout" className="flex items-center" onClick={(e) => { setFocusPage?.('/logout') }}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="-1 0 24 24" strokeWidth="2.5" stroke="currentColor" className={style.sidebarItem}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75" />
                            </svg>
                            <span className={`${isHovered ? style.labelVisible : style.labelHidden}`}>Sair</span>
                        </NavLink>
                    </li>
                </ol>
            </footer>
